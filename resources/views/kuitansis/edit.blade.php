...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('kuitansis.show', $kuitansi->id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 transition-colors">
            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Kuitansi</h1>
            <p class="mt-1 text-sm text-gray-500">{{ $kuitansi->no_kuitansi ?? 'KTN-' . str_pad($kuitansi->id, 4, '0', STR_PAD_LEFT) }}</p>
        </div>
    </div>

    <form action="{{ route('kuitansis.update', $kuitansi->id) }}" method="POST" x-data="kuitansiEditForm()">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="id_invoice" name="id_invoice"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="nama_pelanggan" name="nama_pelanggan"
                            value="{{ old('nama_pelanggan', $kuitansi->nama_pelanggan) }}"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="alamat" name="alamat" rows="2"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Masukkan alamat pelanggan">{{ old('alamat', $kuitansi->alamat) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="no_telp" name="no_telp"
                            value="{{ old('no_telp', $kuitansi->no_telp) }}"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Tambahkan catatan atau keterangan (opsional)">{{ old('keterangan', $kuitansi->keterangan) }}</textarea>
                </div>
            </div>
        </x-ui.card>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('kuitansis.show', $kuitansi->id) }}" 
...
<script>
    function kuitansiEditForm() {
        return {
            totalBayar: {{ old('total_bayar', $kuitansi->total_bayar) }},
            totalDisplay: '{{ number_format(old('total_bayar', $kuitansi->total_bayar), 0, ',', '.') }}',
            items: [
                @foreach($kuitansi->detailKuitansis as $detail)
                {
                    id: {{ $detail->id }},
                    nama_item: '{{ $detail->id_txtKtl }}',
                    jumlah: {{ $detail->jumlah }},
                    harga_satuan: {{ $detail->harga_satuan }},
                    harga_display: '{{ number_format($detail->harga_satuan, 0, ',', '.') }}',

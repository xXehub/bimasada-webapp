    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4">
            ...
            <nav class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                ...
                <a href="{{ route('surat-perjanjians.show', $suratPerjanjian) }}" class="hover:text-primary-600 dark:hover:text-primary-400 transition-colors">PKS-{{ str_pad($suratPerjanjian->id, 4, '0', STR_PAD_LEFT) }}</a>
                <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit PKS</h1>
                        <p class="text-gray-500 dark:text-gray-400 mt-0.5">{{ $suratPerjanjian->nama_pelanggan }}</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2">
                    <x-ui.button variant="ghost" href="{{ route('surat-perjanjians.show', $suratPerjanjian) }}" size="sm">
                        <x-slot name="icon">
        ...
        <div class="flex items-center gap-3">
            ...
            @php
                $statusColors = [
                    ...
                    'Kadaluarsa' => 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
                    'Dibatalkan' => 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
                ];
            @endphp
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-sm font-semibold {{ $statusColors[$suratPerjanjian->status_surat] ?? $statusColors['Draft'] }}">
        ...
        <form action="{{ route('surat-perjanjians.update', $suratPerjanjian) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <x-ui.card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <x-ui.input 
                        ...
                        name="tanggal_surat" 
                        label="Tanggal Surat" 
                        :value="old('tanggal_surat', $suratPerjanjian->tanggal_surat->format('Y-m-d'))" 
                        required 
                    />
                    ...
                    <x-ui.input 
                        ...
                        :value="old('tanggal_selesai', $suratPerjanjian->tanggal_selesai->format('Y-m-d'))" 
                        required 
                    />
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="Draft" {{ old('status_surat', $suratPerjanjian->status_surat) == 'Draft' ? 'selected' : '' }}>Draft</option>
                            <option value="Aktif" {{ old('status_surat', $suratPerjanjian->status_surat) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="Disetujui" {{ old('status_surat', $suratPerjanjian->status_surat) == 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                            <option value="Kadaluarsa" {{ old('status_surat', $suratPerjanjian->status_surat) == 'Kadaluarsa' ? 'selected' : '' }}>Kadaluarsa</option>
                            <option value="Dibatalkan" {{ old('status_surat', $suratPerjanjian->status_surat) == 'Dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-3">
                        ...
                        >
                            ...
                            @foreach($salesList as $s)
                                <option value="{{ $s->id }}" {{ old('id_sales', $suratPerjanjian->id_sales) == $s->id ? 'selected' : '' }}>
                                    {{ $s->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <x-ui.input 
                            ...
                            :value="old('nama_pelanggan', $suratPerjanjian->nama_pelanggan)" 
                            required 
                        >
                            <x-slot name="icon">
                    ...
                    <x-ui.input 
                        ...
                        :value="old('email_pelanggan', $suratPerjanjian->email_pelanggan)" 
                    >
                        <x-slot name="icon">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <x-ui.input 
                        ...
                        :value="old('no_telp_pelanggan', $suratPerjanjian->no_telp_pelanggan)" 
                    >
                        <x-slot name="icon">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            >{{ old('alamat_pelanggan', $suratPerjanjian->alamat_pelanggan) }}</textarea>
                        </div>
                    </div>
                </div>
            </x-ui.card>
            ...
            <x-ui.card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <x-ui.input 
                        ...
                        :value="old('nama_pihak_pertama', $suratPerjanjian->nama_pihak_pertama)" 
                        required 
                    >
                        <x-slot name="icon">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <x-ui.input 
                        ...
                        :value="old('nama_pihak_kedua', $suratPerjanjian->nama_pihak_kedua)" 
                        required 
                    >
                        <x-slot name="icon">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <x-ui.card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-ui.input 
                        ...
                        placeholder="0" 
                        :value="old('nilai_kontrak', $suratPerjanjian->nilai_kontrak)" 
                        required 
                    >
                        <x-slot name="icon">
                    ...
                    <div class="flex items-end">
                        <div class="p-4 rounded-xl bg-gradient-to-br from-primary-50 to-emerald-50 dark:from-primary-900/20 dark:to-emerald-900/20 border border-primary-100 dark:border-primary-800 w-full">
                            <div class="flex items-center gap-2 text-sm text-primary-700 dark:text-primary-300">
                                ...
                                Perbarui nilai kontrak jika diperlukan
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <x-ui.card>
                ...
                <div>
                    ...
                    >{{ old('syarat_ketentuan', $suratPerjanjian->syarat_ketentuan) }}</textarea>
                </div>
            </x-ui.card>
